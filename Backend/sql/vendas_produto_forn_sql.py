...
Agrupa vendas por vendedor + cliente + produto.

- build_vendas_produto_sql: por mês inteiro OU por intervalo (semana)
- build_vendas_produto_semanal_sql: mês inteiro quebrado por semana
  (domingo a sábado), com coluna identificando a semana — usado no Excel.
"""
from typing import List, Optional
from config import FILIAL


def build_vendas_produto_sql(codfornecs: List[int], mes_ref: str,
                              dt_ini: Optional[str] = None,
                              dt_fim: Optional[str] = None) -> tuple:
    """
    codfornecs : lista de CODFORNECs
    mes_ref    : 'YYYY-MM'
    dt_ini/dt_fim : 'YYYY-MM-DD' opcionais — se informados, filtra pelo
                    intervalo (semana) em vez do mês inteiro.
    """
    if not codfornecs:
    ...
    FROM DUAL"""
        ...
        periodo = """
    SELECT TRUNC(TO_DATE(:mes_ref,'YYYY-MM'),'MM')           AS DT_INI,
           LAST_DAY(TRUNC(TO_DATE(:mes_ref,'YYYY-MM'),'MM')) AS DT_FIM
    FROM DUAL"""

    sql = f"""
WITH PARAMS AS ({periodo})
SELECT
...
def build_vendas_produto_semanal_sql(codfornecs: List[int], mes_ref: str) -> tuple:
    """
    Mês inteiro, mas agrupando também por semana (domingo a sábado).
    Cada linha traz semana_ini / semana_fim / semana_label além dos dados.
    Usado para o export Excel de todas as semanas do mês.
    """
    if not codfornecs:
        return "SELECT 1 FROM DUAL WHERE 1=0", {}
    ...
    # TRUNC(data,'D') no Oracle (NLS pt-BR) volta o domingo da semana.
    # semana_ini = domingo; semana_fim = sábado (domingo + 6).
    sql = f"""
WITH PARAMS AS (
    SELECT TRUNC(TO_DATE(:mes_ref,'YYYY-MM'),'MM')           AS DT_INI,
           LAST_DAY(TRUNC(TO_DATE(:mes_ref,'YYYY-MM'),'MM')) AS DT_FIM
...
SELECT
    TRUNC(N.DTSAIDA,'D')                                     AS semana_ini,
    TRUNC(N.DTSAIDA,'D') + 6                                 AS semana_fim,
    N.CODUSUR                                                AS cod_vendedor,
    U.NOME                                                   AS nome_vendedor,
    M.CODCLI                                                 AS cod_cliente,
    NVL(NULLIF(TRIM(C.FANTASIA),''), C.CLIENTE)              AS nome_cliente,
...
GROUP BY
    ...
    M.CODCLI, C.FANTASIA, C.CLIENTE,
    M.CODPROD, PR.DESCRICAO
ORDER BY TRUNC(N.DTSAIDA,'D'), U.NOME, C.CLIENTE, PR.DESCRICAO
"""
    return sql, params