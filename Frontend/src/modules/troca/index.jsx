...
const pctTroca = (v) => {
  ...
  return v >= 2
    ? { label: `${v.toFixed(1)}%`, style: { color: "#fff", background: C.red, borderRadius: "3px", padding: "1px 5px", fontWeight: 600 } }
    : { label: `${v.toFixed(1)}%`, style: { color: "#fff", background: C.green, borderRadius: "3px", padding: "1px 5px", fontWeight: 600 } };
};
...
const pctFlexTroca = (v) => {
  ...
  return val > 2
    ? { label: `${val.toFixed(1)}%`, style: { color: "#fff", background: C.red, borderRadius: "3px", padding: "1px 5px", fontWeight: 600 } }
    : { label: `${val.toFixed(1)}%`, style: { color: "#fff", background: C.green, borderRadius: "3px", padding: "1px 5px", fontWeight: 600 } };
};
...
function EquipeTrocaCard({ supervisor, rows, isMobile }) {
  ...
  return (
    <div style={{ margin: "12px 16px", background: "#fff", border: `1px solid ${C.border}`,
      borderRadius: "6px", boxShadow: "0 1px 6px rgba(170,0,0,.1)", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
        padding: "8px 14px", background: `linear-gradient(90deg,${C.header},${C.primary})`, cursor: "pointer" }}
      ...
      {aberto && (
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: isMobile ? "11px" : "12px" }}>
            <thead>
              <tr>
                <Th label="RCA" align="center" /><Th label="VENDEDOR" align="left" />
                <Th label="PLF TOTAL" /><Th label="TROCA" /><Th label="% TROCA" align="center" />
                <Th label="FLEX" /><Th label="% TROCA - FLEX" align="center" />
              </tr>
            </thead>
            <tbody>{supRows.map((r, i) => <DataRow key={r.cod_vendedor ?? i} row={r} i={i} showVendedor={true} />)}</tbody>
            <tfoot>
...
export default function ModuleTroca({ isMobile, token, userInfo = {} }) {
  ...
  const modoInicial = cargo === "vendedor" ? "vendedor" : cargo === "supervisor" ? "equipe" : "todas_equipes";
  ...
  return (<>
    ...
    {mode !== "todas_equipes" && (
      <div ref={tableRef} style={{ margin: isMobile ? "8px" : "12px 16px", background: "#fff",
        border: `1px solid ${C.border}`, borderRadius: "4px", overflow: "hidden", boxShadow: "0 1px 6px rgba(170,0,0,.12)" }}>
        {loading && <div style={{ padding: "48px", textAlign: "center", color: C.textSub }}>Carregando...</div>}
        {error   && <div style={{ padding: "24px", textAlign: "center", color: C.red }}>{error}</div>}
          ...
          <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: isMobile ? "11px" : "12px" }}>
              ...
              <tbody>
                ...
                {rows.length === 0 && <tr><td colSpan={showVendedor ? 7 : 5} style={{ padding: "32px", textAlign: "center", color: C.textSub }}>Nenhum dado encontrado.</td></tr>}
              </tbody>
              {rows.length > 0 && (
                <tfoot>
                  <tr style={{ background: C.total, color: C.totalTxt, fontWeight: 700 }}>
                    {showVendedor ? <td colSpan={2} style={{ padding: "5px 8px", border: "1px solid #880000" }}>TOTAL</td> : <td style={{ padding: "5px 8px", border: "1px solid #880000" }}>TOTAL</td>}
                    <td style={{ padding: "5px 8px", border: "1px solid #880000", textAlign: "right", fontFamily: C.mono }}>{fmtR(tot.plf_total)}</td>
                    <td style={{ padding: "5px 8px", border: "1px solid #880000", textAlign: "right", fontFamily: C.mono }}>{fmtR(tot.troca)}</td>
    ...
    {isAdmin && showDebug && rows.length > 0 && (
      <div style={{ margin: "0 16px 16px", background: "#fff", border: "1.5px solid #7C3AED",
        ...
        <div style={{ padding: "8px 14px", background: "#7C3AED", color: "#fff", fontWeight: 700, fontSize: "12px", display: "flex", alignItems: "center", gap: "8px" }}>
          🔬 Debug SQL — valores intermediários
        </div>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11px" }}>
            ...
            <tbody>
              {rows.map((r, i) => {
                const pt2 = pctTroca(r.pct_troca);
                const pft2 = pctFlexTroca(r.pct_flex_troca);
                return (
